#!/usr/bin/env bash
# Alle One — REINÍCIO PRODUÇÃO (suporte)
# Uso (na VM, como alleone ou com sudo -u alleone):
#   bash /home/alleone/producao/deploy/scripts/restart-alleone-prod.sh
#
# Não faz git pull nem build — só sobe/reinicia os processos PM2.
# Se o site “caiu”, este é o primeiro script a rodar.
set -euo pipefail

ROOT="${ALLEONE_ROOT:-/home/alleone/producao}"
API_NAME="${ALLEONE_API_NAME:-alleone-api}"
...
echo "==> Alle One PRODUÇÃO — restart"
echo "    root: $ROOT"
...
if ! curl -sf "$API_HEALTH_URL" >/dev/null; then
  echo "FALHA: API não respondeu 200 em $API_HEALTH_URL"
...
if [[ "$WEB_CODE" != "200" && "$WEB_CODE" != "307" && "$WEB_CODE" != "302" ]]; then
  echo "AVISO: web não retornou 200/302/307 — verifique logs"
