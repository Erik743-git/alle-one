...
test.describe('Aprovação de GMUD', () => {
  ...
  test('carrega a fila ou aplica o bloqueio de permissão', async ({ page }) => {
    await login(page);
    await page.goto('/gmud');

    const heading = page.getByRole('heading', { name: 'GMUD', exact: true });
    ...
    if (await heading.isVisible()) {
      ...
      await expect(page.getByText('Lista de GMUDs', { exact: true })).toBeVisible();
      return;
    }
