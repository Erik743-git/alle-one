...
function TimeInputs({
  ...
  const [hourDraft, setHourDraft] = React.useState(() =>
    String(Number.parseInt(hour || "09", 10)),
  );
  const [minuteDraft, setMinuteDraft] = React.useState(() =>
    String(Number.parseInt(minute || "00", 10)),
  );

  React.useEffect(() => {
    setHourDraft(String(Number.parseInt(hour || "09", 10)));
    setMinuteDraft(String(Number.parseInt(minute || "00", 10)));
  }, [hour, minute]);
  ...
  const commit = React.useCallback(
    (nextHour: string, nextMinute: string) => {
      ...
      onTimeChange(`${hh}:${mm}`);
      setHourDraft(String(Number.parseInt(hh, 10)));
      setMinuteDraft(String(Number.parseInt(mm, 10)));
    },
    [onTimeChange],
  );
